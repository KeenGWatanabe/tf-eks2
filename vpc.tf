terraform {
  backend "s3" {
    bucket         = "vpc-eks.tfstate-backend.com"  # Must match the bucket name above
    key            = "vpc-eks/terraform.tfstate"        # State file path
    region         = "us-east-1"                # Same as provider
    dynamodb_table = "vpc-eks-terraform-state-locks"    # If using DynamoDB
